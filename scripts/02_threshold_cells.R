...
# Step 7. Annotate cells with the corresponding cell type, threshold label, and cell barcode.
    AUC_mat$celltype = threshold_grid[i,2]
    AUC_mat$percent = threshold_grid[i,1]
    AUC_mat$cell = rownames(AUC_mat)
    print(i)
# Step 8. Save the threshold-specific AUCell classification results.
    saveRDS(AUC_mat, paste0("path", threshold_grid[i,2],"_146_AUC_mat_top_", threshold_grid[i,1],".RDS"))  
# Step 9. Append results into a cumulative table for downstream pseudobulk aggregation and DE analysis.
